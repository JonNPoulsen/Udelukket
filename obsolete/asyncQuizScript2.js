<script>
		...
		const customCategories = <%- JSON.stringify(selectedCategories) %>;
		const customQuestions = [];

		if (allCategoriesOn === false) {
			for (var i = 0; i < qJSON.length; i++) {
				if (customCategories.includes(qJSON[i].category) || customCategories.includes(qJSON[i].category_2)) 
				{
					customQuestions.push(qJSON[i]);
				}
			}
		}
